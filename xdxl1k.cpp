...
inline void insert(int poi)
{
    ...
    while(ilen>=1&&si)
    {
        if(h[ilen>>1].v>h[ilen].v) swap(h[ilen],h[ilen>>1]);
        else si=0; ilen>>=1;
    }
}
inline void destory(int poi)
{
    ...
    while((ilen<<1)<=len&&si)
    {
        ...
        {
            if(h[ilen<<1].v>h[ilen<<1|1].v&&(ilen<<1|1)<=len) t=ilen<<1|1;
            else t=ilen<<1; swap(h[ilen],h[t]); ilen=t;
        }
        else si=0;
    }
}
